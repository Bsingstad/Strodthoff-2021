...
class Transformer(nn.Module):
   def __init__(self, patch_size, input_shape, emb_dim_enc=128, num_layers=4, nhead=4, dropout=0.1, 
                dim_feedforward=1024, num_classes=1):
...
class StftTransform(nn.Module):
   ...
   def forward(self, x):
       ...
       for c in range(x.shape[0]):
           stft_channels.append(torch.stft(x[c], self.n_fft, self.hop_length, self.win_length,
                                           self.window, return_complex=True, center=False))
